...
public class GameLogic implements PlayableLogic {
    private ConcretePiece[][] board = new ConcretePiece[11][11]; //the main board of the game
    private ArrayList<ConcretePiece> [][] positionBoard = new ArrayList [11][11]; //this board is meant to help us know which soldiers stepped on any given square during the game
    private Position [][] position = new Position[11][11]; //We also set up a table of positions to help us later save for each position which tools stepped on it
    private ArrayList<Position> listPosition= new ArrayList<>();   // we saved all the soliders that step on the position
    ...
    public boolean move(Position a, Position b) {
        ...
        boolean isAttackerrTurn = isSecondPlayerTurn(); // player2 is the attacker
        boolean defenderPosition = getPieceAtPosition(a).getOwner().isPlayerOne(); // want to check if its piece on the position is belongs to defender

        if ((isAttackerrTurn && defenderPosition) || (!isAttackerrTurn && !defenderPosition)) // We want to check that the pawn the player wants to move belongs to his team
            return false;
        if (!legal(a, b))
            return false;
